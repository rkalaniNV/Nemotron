...
def run(cfg: dict, started_at: str | None = None) -> dict[str, Any]:
    ...
    if fmt == "auto":
        ...
        raise ingest_module.IngestError(
            f"format must be one of {ingest_module.FORMATS} or 'auto', got {fmt!r}"
        )
    ...
    id_fields = list(cfg.get("id_fields") or [f for f in (cfg.get("url_from"), text_field) if f])
    id_prefix = cfg.get("id_prefix") or ""
    keep = list(cfg.get("keep_fields") or [])

    output_dir.mkdir(parents=True, exist_ok=True)
    for stale in output_dir.glob("part_*.jsonl"):
        stale.unlink()
    ...
    shard_index = 0
    in_shard = 0
    handle = (output_dir / f"part_{shard_index}.jsonl").open("w", encoding="utf-8")
    try:
        for path in paths:
            for raw in reader(path):
                ...
                doc_id = record["id"]
                if doc_id in seen:
                    # Same id means byte-identical content, because that is what
                    # the id is derived from. Which of the three answers is right
                    # depends on the corpus, so the caller has already chosen.
                    ...
                    if on_duplicate == "suffix":
                        seen[doc_id] += 1
                        record["id"] = f"{doc_id}-{seen[doc_id]}"
                    # "refuse" is decided after the pass, so the report can say
                    # how many there were rather than only the first.
                else:
                    seen[doc_id] = 0
    ...
    if stats["duplicate_ids"] and on_duplicate == "refuse":
        raise ingest_module.IngestError(
            f"{stats['duplicate_ids']} document(s) are byte-identical to an earlier one, in "
            f"{len(duplicate_groups)} group(s) (e.g. {', '.join(duplicate_examples)}). A "
            "content-derived id cannot tell them apart, and "
            "subset and decontamination both refuse a non-unique id. Choose: "
    ...
    report = {
        ...
        "identity": {
            ...
            "note": (
                ...
                if id_field
                else "Ids are derived from content, so they survive resharding — unlike a "
                "positional id, which renames every document when the corpus is re-split."
            ),
        },
    ...
    if stats["duplicate_ids"]:
        report["warnings"].append(
            f"{stats['duplicate_ids']} byte-identical document(s) in {len(duplicate_groups)} "
    ...
    print(
        f"curate/ingest: {stats['written']:,} document(s) from {len(paths)} {fmt} file(s) "
        f"-> {output_dir}"
    )
    return report
