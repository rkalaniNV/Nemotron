...
class LongContextEpisodeGenerator(
    ...
    def generate(self, data: dict) -> dict:
        ...
        try:
            ...
            seed = EpisodeSeed.model_validate_json(raw) if isinstance(raw, str) else EpisodeSeed.model_validate(raw)
            ...
            record = EpisodeRunner(cfg).run(models, seed, registry, run_id=self.config.run_id)
        except Exception as exc:
            record = CanonicalRecord(
                run_id=self.config.run_id,
                config_fingerprint=cfg.fingerprint() if cfg is not None else "unavailable",
        ...
        data["trajectory_validation"] = json.dumps(record.validation, ensure_ascii=False)
