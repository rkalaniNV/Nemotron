...
# name = "lightning35/data/prep/pretrain"
...
"""Data preparation for Lightning35 pretraining stage.
...
CLI:
    nemotron lightning35 data prep pretrain                       # local execution
    nemotron lightning35 data prep pretrain --run ray --sample 10000  # submit to cluster

Execution logic: src/nemotron/cli/commands/lightning35/data/prep/pretrain.py
...
class PreTrainDataPrepConfig:
    ...
    output_dir: Path = field(default_factory=lambda: _OUTPUT_BASE / "output/lightning35/stage0_pretrain")
...
def run_data_prep_main(
    ...
    if blend.datasets is None:
        raise ValueError(
            f"Lightning35 pretrain expects single-blend format (datasets list), "
    ...
    artifact_name = f"lightning35/pretrain/data{config_suffix}{sample_suffix}"
