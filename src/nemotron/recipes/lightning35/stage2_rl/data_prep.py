...
"""Data preparation for Lightning35 RL stage.

Processes the nvidia/Nemotron-3.5-Lightning-RL-Training-Blend dataset and resolves
...
CLI:
    nemotron lightning35 data prep rl                       # local execution
    nemotron lightning35 data prep rl --run ray --sample 10000  # submit to cluster

Execution logic: src/nemotron/cli/commands/lightning35/data/prep/rl.py
...
class RLDataPrepConfig:
    ...
    Processes nvidia/Nemotron-3.5-Lightning-RL-Training-Blend and resolves placeholder
    ...
    output_dir: Path = field(default_factory=lambda: _OUTPUT_BASE / "output/lightning35/stage2_rl_resolved")
    ...
    # Config variant name (e.g. "tiny"); suffixes the artifact name so
    # non-default pipeline chains stay isolated (mirrors stage0/stage1).
...
def run_data_prep_main(
    ...
    artifact.name = f"lightning35/rl/data{config_suffix}{'?sample=' + str(cfg.sample) if cfg.sample else ''}"
