...
# Default worker-side paths for offline installs and preseeded repository
# caches. Executor profiles should normally override these with pip_wheelhouse
# or repo_cache_root. The defaults match this repository's current deploy image
# layout, but the public nemo_runspec knobs are intentionally generic.
DEFAULT_OFFLINE_WHEELHOUSE = "/opt/nemotron-airgap/wheels"
DEFAULT_PRESEEDED_REPO_ROOT = "/opt/nemotron-airgap/assets/repos"
GENERIC_REPO_CACHE_ENV = "NEMO_RUNSPEC_REPO_CACHE"
LEGACY_REPO_CACHE_ENV = "NEMOTRON_AIRGAP_REPOS"
PRESEEDED_REPO_ROOT_EXPR = (
    f"${{{GENERIC_REPO_CACHE_ENV}:-${{{LEGACY_REPO_CACHE_ENV}:-{DEFAULT_PRESEEDED_REPO_ROOT}}}}}"
)
...
def _resolve_preseeded_repo_root(tunnel: Any, *, env: Any, override: str | None) -> str | None:
    """Pick the preseeded git-cache root to honor for ``auto_mount`` repos.

    Priority order, first hit wins:

    1. Explicit ``override`` argument from the caller.
    2. Generic executor profile fields such as ``repo_cache_root``.
    3. Generic ``env.env_vars.NEMO_RUNSPEC_REPO_CACHE``.
    4. Legacy ``NEMOTRON_AIRGAP_REPOS`` aliases for existing Nemotron profiles.
    5. Best-effort SSH login-shell environment lookup.
    """
    if override:
        value = str(override).strip()
        if value:
            _log.debug("Using preseeded repo root from explicit override: %s", value)
            return value
    if env is not None:
        for field in ("repo_cache_root", "preseeded_repo_root", "offline_repo_root", "airgap_repos"):
            direct = _get_env(env, field)
            if direct:
                value = str(direct).strip()
                if value:
                    _log.debug("Using preseeded repo root from executor profile field %s: %s", field, value)
                    return value
        env_vars = _get_env(env, "env_vars")
        if env_vars is not None:
            plain_env_vars = _to_plain(env_vars)
            if isinstance(plain_env_vars, dict):
                for key in (GENERIC_REPO_CACHE_ENV, LEGACY_REPO_CACHE_ENV):
                    value = plain_env_vars.get(key)
                    if value:
                        resolved = str(value).strip()
                        if resolved:
                            _log.debug("Using preseeded repo root from executor env var %s: %s", key, resolved)
                            return resolved
    for key in (GENERIC_REPO_CACHE_ENV, LEGACY_REPO_CACHE_ENV):
        ...
        if local:
            value = local.strip()
            if value:
                _log.debug("Using preseeded repo root from local env var %s: %s", key, value)
                return value
    try:
        lookup_cmd = f"printenv {GENERIC_REPO_CACHE_ENV} || printenv {LEGACY_REPO_CACHE_ENV}"
        result = tunnel.run(lookup_cmd, hide=True, warn=True)
    except Exception:
        _log.debug("No preseeded repo root configured; remote environment lookup failed", exc_info=True)
        return None
    remote = (result.stdout or "").strip() if getattr(result, "ok", False) else ""
    if remote:
        _log.debug("Using preseeded repo root from remote executor environment: %s", remote)
        return remote
    _log.debug("No preseeded repo root configured for auto_mount repos")
    return None
    ...
    *,
    ...
    Args:
        ...
        env: Optional executor profile (OmegaConf or mapping). When provided we
            read a preseeded repo cache root from generic fields/env vars. This
            remains positional-or-keyword for compatibility with older callers.
        repo_cache_root: Explicit generic override; takes precedence over ``env``.
        airgap_repos: Legacy alias for ``repo_cache_root``.
    ...
    resolved_repo_root = _resolve_preseeded_repo_root(
        tunnel,
        env=env,
        override=repo_cache_root or airgap_repos,
    )
    ...
    for repo_name, repo_info in git_mounts.items():
        ...
        if resolved_repo_root and target:
            preseeded_repo = f"{resolved_repo_root.rstrip('/')}/{repo_name}"
            result = tunnel.run(f"test -d {preseeded_repo}/.git && echo exists", hide=True, warn=True)
            if result.ok and "exists" in result.stdout:
                typer.echo(f"[auto_mount] Using preseeded repo {preseeded_repo}...")
...
RAY_HEAD_RUNTIME_PACKAGES = ("typer", "rich", "pydantic-settings", "shellingham")
...
def _python_import_check_command(import_names: Iterable[str], *, context: str) -> str | None:
    ...
    if not modules:
        return None
    message_prefix = f"{context} missing Python imports: "
    code = (
        "import importlib.util\n"
        f"mods = {modules!r}\n"
        f"message_prefix = {message_prefix!r}\n"
        "missing = [m for m in mods if importlib.util.find_spec(m) is None]\n"
        "if missing:\n"
        "    raise SystemExit(message_prefix + ', '.join(missing))\n"
    )
...
def _remote_pip_setup_commands(
    ...
    *,
    ...
    if mode == "offline_wheelhouse":
        # Profiles should set pip_wheelhouse explicitly. The fallback matches
        # the current deploy image layout for backwards-compatible smoke tests.
        wheelhouse = str(_get_env(env, "pip_wheelhouse") or DEFAULT_OFFLINE_WHEELHOUSE)
        install_parts = [
            "python",
            "-m",
            "pip",
...
# same offline-aware pip setup without depending on private names. The
...
def _git_mount_commands() -> list[str]:
    ...
    for repo_name, info in get_git_mounts().items():
        ...
        if is_sha:
            ...
            preseeded = f"{PRESEEDED_REPO_ROOT_EXPR}/{repo_name}"
            commands.append(
...
def execute_cloud(
    ...
    *,
    ...
    This is offline-friendly and picks up local uncommitted edits without
    ...
    # Offline profiles should use pip_install_mode=preinstalled or
    # pip_install_mode=offline_wheelhouse. Run this after startup/setup so
    # profiles can load Python modules or configure pip first.
...
def execute_cloud_ray(
    ...
    *,
    ...
    ray_head_packages = [*RAY_HEAD_RUNTIME_PACKAGES, *pip_extras]
    head_setup.extend(_remote_pip_setup_commands(env, ray_head_packages, context=f"{executor_type} Ray head"))
    full_cmd = " && ".join(
        [
            *head_setup,
