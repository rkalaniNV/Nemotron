...
# Config comes from the environment (set by the notebook's configuration cell).
for _required_var in ("MODEL_ID", "MAX_SEQ_LEN", "DATAPREP_OUTPUT_DIR"):
    if _required_var not in os.environ:
        raise RuntimeError(
            f"{_required_var} environment variable must be set (see the notebook's configuration cell)."
        )
