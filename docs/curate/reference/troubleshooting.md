...
| Ray worker starts a new `.venv` or cannot import dependencies | Local `uv run` and Ray runtime environment are fighting over dependency setup | Export `RAY_ENABLE_UV_RUN_RUNTIME_ENV=0` and run with `uv run --no-sync` after `uv sync --extra curate`. |
